...
def load_questions(defect_name, step):
    ...
    for q in questions_data:
        st.session_state.kakunin_koumoku[q["code"]] = "不明"
...
def kakunin_koumoku():
    ...
    if not questions_data:
        ...
        st.button("最初に戻る", on_click=lambda: st.session_state.update(step=1, step1_option=0, defect_name=None, easy_answers={}), type="primary")
        return
...
def koutei_kakunin():
    ...
    if not questions_data:
        st.warning('質問が読み込めませんでした。')
        st.button("最初に戻る", on_click=lambda: st.session_state.update(step=1, step1_option=0, defect_name=None, kakunin_koumoku={}, koutei_kakunin={}), type="primary")
        return
